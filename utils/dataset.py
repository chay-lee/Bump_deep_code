...
class CustomDataset(torch.utils.data.Dataset):
    def __init__(self, root_dir, scan_params, y_range=(0, 10), depth=56, input_type='3d', dataset_type='train'):
        ...
        self.is_recon_only = (config.get("lambda_val_1", 0.0) == 0.0 and config.get("lambda_val_2", 0.0) == 0.0)
        
        self.is_test_mode = (self.dataset_type not in ['train', 'val']) or config.get("repeat_test", False)
        
        if self.dataset_type in os.path.basename(self.root_dir):
            base_root = self.root_dir
        elif config.get("repeat_test", False):  
            base_root = os.path.join(self.root_dir)
        else:
    ...
    def __getitem__(self, idx):
        ...
        if self.is_test_mode or self.is_recon_only:
            ...
            if config.get("rand_5", False) and len(existing_suffixes) > 5:
...
def load_datasets(root_dir, args, is_train):
    if is_train:
        trainset = CustomDataset(root_dir, args['scan_params'], args['y_range'], args['depth'], dataset_type='train')
        valset = CustomDataset(root_dir, args['scan_params'], args['y_range'], args['depth'], dataset_type='val')
        return [trainset, valset]
    else:
        testset = CustomDataset(root_dir, args['scan_params'], args['y_range'], args['depth'], dataset_type='test')
        return testset
