...
def update_args_for_test(checkpoint, current_args):
    """Overwrite current arguments with checkpoint arguments to match training state, 
    while preserving test-specific configurations."""
    checkpoint_args = checkpoint['args']
    
    test_params_to_keep = [
        'device', 'is_train', 'dataset_path', 
        'batch_size_test', 'repeat_test', 'model_mode'
    ]
    
    for key, value in checkpoint_args.items():
        if key not in test_params_to_keep:
...
if __name__ == '__main__':
    ...
    args = parse_args()
    train_module.args = args  # Sync parsed args with the train module
    ...
    print(f"Lambda 1 (Consis): {args.lambda_val_1}, Lambda 2 (TV): {args.lambda_val_2}\n")
    ...
    if args.is_train:
        ...
        shutil.copytree(os.getcwd(), os.path.join(MODEL_DIR, 'codes'))
        ...
        train_loader, val_loader = make_dataloaders(dataset, args.batch_size, is_train=True)
        ...
        with open(os.path.join(MODEL_DIR, "training_log.txt"), log_mode) as txt_file:
            if log_mode == 'a':
                ...
                txt_file.write(f'\n\n[INFO] Training resumed from Epoch {start_epoch} at {current_time_str}\n')

            train_module.train(train_loader, val_loader, model, txt_file, loss_fn, optimizer, MODEL_DIR, lr_scheduler, start_epoch)
    ...
    # ==========================================
        ...
        # Load hyperparameters from checkpoint to ensure consistent testing
        args = update_args_for_test(checkpoint, args)
        train_module.args = args  # Re-sync after updating args from checkpoint
        ...
        if getattr(args, 'repeat_test', False):
            ...
            for scan_id in range(1, 11):
                output_dir = os.path.join(results_root, 'repeat_test', str(scan_id))
                run_test(model, args, str(scan_id), output_dir, checkpoint)
        else:
            output_dir = os.path.join(results_root, 'test')
            run_test(model, args, 'test', output_dir, checkpoint)

        calculate_and_save_mae_stats(base_dir=results_root, mae_save_dir=results_root)
        calculate_and_save_bump_stats(base_dir=results_root, save_dir=results_root)
